...
const CateringPage = () => {
  ...
  const handleOrder = async () => {
    if (cart.length === 0) {
      toast.error("Please add items to your catering order");
      return;
    }
    if (!date || !time) {
    ...
    try {
      ...
      if (code === "LESS_THAN_48_HOURS") {
        ...
        toast.error(
          "One or more items are not available for catering. Please refresh and try again."
        );
      ...
      <div className="container max-w-lg py-20 text-center">
        <div className="rounded-xl border border-border bg-card p-8">
          ...
          <p className="mt-3 text-sm font-mono text-primary tracking-widest">
            {cateringResult.reference_number}
          </p>
          ...
          <div className="mt-4 rounded-lg bg-secondary p-4 text-sm text-foreground space-y-1">
            <p>
              Order total:{" "}
              <span className="font-semibold">${cateringResult.total_amount.toFixed(2)}</span>
            </p>
            <p>
              Deposit required:{" "}
              <span className="font-semibold text-primary">
                ${cateringResult.deposit_amount.toFixed(2)}
              </span>
            </p>
          </div>
          <p className="mt-3 text-xs text-muted-foreground">
            Our team will contact you within 24 hours to arrange the deposit payment.
          </p>
          <WhatsAppButton
            message={`Hi Aap ki Rasoi! I have a question about my catering order ${cateringResult.reference_number}`}
  ...
  return (
    <div className="container py-12">
      ...
      <p className="mx-auto mt-2 max-w-lg text-center text-muted-foreground">
        Perfect for events, parties, and celebrations. All orders must be placed at least 48 hours
        in advance.
      </p>
          ...
          return (
            <div key={item.id} className="rounded-xl border border-border bg-card p-4">
              ...
              <p className="text-sm text-primary font-bold">
                ${item.catering_price_per_tray?.toFixed(2)} / tray
              </p>
        ...
        <div className="mx-auto mt-10 max-w-lg space-y-5">
            ...
            {cart.map((ci) => {
              ...
              return (
                <div key={ci.itemId} className="flex justify-between text-sm">
                  <span>
                    {item.name} × {ci.trays} trays
                  </span>
                  <span className="font-medium">
                    ${((item.catering_price_per_tray || 0) * ci.trays).toFixed(2)}
                  </span>
                </div>
              );
            })}
              ...
              <Popover>
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    ...
                    selected={date}
                    onSelect={setDate}
                    disabled={(d) => d < addHours(new Date(), 48)}
                    initialFocus
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="e.g. vegetarian guests only, no nuts (optional)"
            />
          </div>

          <p className="text-xs text-muted-foreground">
            ⚠️ All catering orders must be placed at least 48 hours in advance.
          </p>
