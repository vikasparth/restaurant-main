    ...
    <>
      ...
      <div className="fixed right-0 top-0 z-50 flex h-full w-full max-w-md flex-col bg-background shadow-xl animate-slide-in-right">
        <div className="flex items-center justify-between border-b border-border p-4">
          ...
          <button onClick={() => setIsCartOpen(false)} className="rounded-full p-1 hover:bg-secondary" aria-label="Close cart">
            <X className="h-5 w-5" />
          </button>
        </div>
            ...
            <div className="space-y-4">
              {items.map((ci) => (
                <div key={ci.item.id} className="flex gap-3 rounded-lg border border-border p-3">
                  <img src={ci.item.image} alt={ci.item.name} className="h-16 w-16 rounded-md object-cover" loading="lazy" />
                  <div className="flex-1">
                    <h3 className="text-sm font-medium text-foreground">{ci.item.name}</h3>
                    <p className="text-sm font-semibold text-primary">${ci.item.price.toFixed(2)}</p>
                    <div className="mt-1 flex items-center gap-2">
                      ...
                      <button onClick={() => removeItem(ci.item.id)} className="ml-auto text-destructive hover:opacity-70">
