...
def _parse_code_fix(response) -> dict | None:
    for part in response.content:
        if getattr(part, "type", "") == "tool_use" and getattr(part, "name", "") == "return_code_fix":
            return part.input
    return None
    ...
    return [
        {
            ...
            "input_schema": {
                ...
                "required": [
                    "root_cause", "affected_layer", "regression", "confidence",
                    "recommended_fix", "original_snippet", "replacement_snippet",
                ],
            },
        }
    ]

def _commit_and_push(file_path: str, patched_content: str, branch_name: str,
                     commit_message: str) -> tuple[str | None, str | None]:
    ...
    if result.returncode != 0:
        subprocess.run(["git", "checkout", GITHUB_BRANCH], capture_output=True, text=True)
...
def _format_pr_body(
    ...
    if remaining_files:
        ...
        for f in remaining_files:
            lines.append(f"- `{f}`")
    sources = [k for k in ("sentry_frontend", "sentry_backend", "render_logs", "github") if k in payload]
    if sources:
        lines += ["", "## Evidence Sources", ", ".join(sources)]
    if issue_number:
        lines += ["", f"Closes #{issue_number}"]
    lines += ["", "---", "🤖 Opened automatically by the Coding Agent. **Draft — do not merge without review.**"]
    return "\n".join(lines)
    
def _open_draft_pr(branch_name: str, interpretation: dict, payload: dict,
                    file_changed: str, remaining_files: list[str], issue_number: str) -> str | None:
    pr_title = f"Fix: {interpretation.get('recommended_fix', 'automated fix')[:60]}"
    pr_body = _format_pr_body(interpretation,payload,file_changed,remaining_files, issue_number)
...
def run(payload: dict, issue_number: str = "") -> dict:
    ...
    if validation_error:
        return _return({"status": STATUS_INVALID_INPUT, "source": "coding", "error": validation_error})

    # ── environment check ─────────────────────────────────────────────────────
    env_error = _check_environment()
    if env_error:
        return _return({"status": STATUS_INVALID_INPUT, "source": "coding", "error": env_error})
    ...
    try:
        ...
        if code == 401:
            ...
            status = STATUS_SERVER_ERROR
        return _return({"status": status, "source": "coding", "error": f"Claude API error with status code {code}."})
    except anthropic.APITimeoutError:
        return _return({"status": STATUS_TIMEOUT, "source": "coding", "error": "Request to Claude API timed out."})
    except anthropic.APIConnectionError:
        return _return({"status": STATUS_NETWORK_ERROR, "source": "coding", "error": "Network error while connecting to Claude API."})
    ...
    _apply_patch_result = _apply_patch(file_content, fix["original_snippet"], fix["replacement_snippet"])
    if _apply_patch_result is None:
        return _return({"status": STATUS_PARTIAL, 
                        "source": "coding",
                        "interpretation": {
    ...
    if not _should_open_pr(fix["confidence"]):
        return _return({"status": STATUS_COMPLETED, 
                        "source": "coding",
        ...
        commit_sha, partial_code = _commit_and_push(fix_files[0], _apply_patch_result, branch_name, commit_message)
        ...
        if commit_sha is not None:
            pr_url = _open_draft_pr(branch_name, interpretation, payload, fix_files[0], fix_files[1:], issue_number)
            if pr_url is None:
                return _return({"status": STATUS_PARTIAL, 
                                "source": "coding",
                                "interpretation": interpretation,
