...
## API Integration Tests — Shallow Happy-Path Coverage Misses Critical Failure Modes

When the D.4 GitHub Extractor test plan was first written, it contained around eight tests covering only the basic happy path and a handful of input cases. Left unchallenged, the implementation would have shipped with no tests for authentication failure, authorization scope errors, rate limiting, server and network faults, response schema violations, or pagination boundary conditions. These are precisely the scenarios where an integration either handles failure gracefully and keeps the system stable, or fails ungracefully and surfaces as an undiagnosed production incident.

The deeper problem is systemic: without a shared checklist, coverage quality depends entirely on whether the engineer or GenAI agent writing the tests happens to think of each category. That reliance is fragile — the same gaps will recur on every new integration. The fix was to encode the required categories into a reusable skill (`/api-integration-tests`) so that any developer or agent invoking it gets a complete, opinionated test plan for any HTTP API integration, with all eight mandatory categories (plus a conditional pagination category) generated from the spec before a single line of test code is written. More skills of this kind should be built as similar patterns emerge.

**Guardrail created:** API integration test plans must cover all eight categories — happy path, input validation, authentication, authorization, resource not found, rate limiting, server and network failures, and response schema validation — enforced via the `/api-integration-tests` skill, which generates the full test plan from the spec before any implementation is written.

## Agentic Loop — Processed Tool Results Must Be Trimmed Before the Next Turn

In a multi-turn agentic loop, the Anthropic API is stateless — the full `messages` list is resent on every `client.messages.create()` call. When a file read returns several thousand characters of source code, that content travels in every subsequent API call, even though Claude has already processed it. If Claude reads three files across three turns, by turn four the messages list contains all three file contents concatenated — token usage compounds with every turn, not linearly. The D.ST.5 smoke test hit 11,000 input tokens partly because of this: file contents accumulated and were resent each turn untouched.

The correct behaviour is to shrink each tool result's content to a one-liner stub — for example `"[file read — content processed]"` — immediately after the API call that consumed it returns. Claude has already reasoned about the content at that point; keeping the raw text in subsequent messages serves no purpose and costs real money. This is the same "trim at the boundary" principle that applies to external API responses generally: extract only what the model needs, discard the rest. In an agentic loop, "the rest" includes file content that has already been used.

**Guardrail created:** After each `client.messages.create()` call in an agentic loop, shrink tool results whose content has been fully consumed and is not needed for future reasoning. Replace with a short stub like `"[file read — content processed]"`. Do not trim results a later turn may need to compare or reconcile — the right trim boundary depends on what the agent is reasoning about, not a blanket rule.

## Agent-Generated Code Must Follow the Same Engineering Guardrails as Human-Written Code

When designing the Coding Agent (D.6), the initial spec committed code changes via the GitHub Contents API — a server-side operation that bypasses the local git workflow entirely. Pre-commit hooks never fired. The agent was held to a lower standard than any engineer on the team would be: no linting, no style checks, no automated quality gates before the commit landed.

The user caught this through repeated questions about pre-commit hooks. The right fix was to recognise that committing via local `git commit` + `git push` — in any git-enabled environment where the repo is checked out and pre-commit is installed — means hooks fire automatically, exactly as they do for a human developer. The GitHub Contents API approach was a design convenience that quietly eroded a team guardrail. The compute environment an agent runs in is an architectural decision that must account for which team guardrails need to be preserved.

The broader principle is that an agent producing production code is not a special case exempt from team standards — it is a team member held to the same bar. Any workflow an engineer must follow (pre-commit hooks, branch naming conventions, draft PRs, CI gates) applies equally to an agent committing to the same codebase.

**Guardrail created:** When an agent writes and commits code, it must follow the same development workflow as a human engineer on the team — including pre-commit hooks, branch conventions, and CI gates. If the commit mechanism bypasses any of these, redesign the mechanism, not the guardrail. This requires the agent to run in a git-enabled environment where the repo is checked out and pre-commit is installed — not a specific runner, but one with the right capabilities.
